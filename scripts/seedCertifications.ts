...
// Removed external import to prevent module resolution issues
...
async function seedCertifications() {
    console.log(`Starting seed of ${certificationsData.length} certifications...`);

    const dbItems = certificationsData.map(item => ({
        // Map camelCase to snake_case matching the schema
        code: item.code,
        name: item.name,
    ...
    const { error } = await supabase.from('certifications').insert(dbItems);

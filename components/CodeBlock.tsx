  ...
  rules.forEach(rule => {
    ...
    parts.forEach((part, partIndex) => {
      if (typeof part === 'string') {
        // Split with capturing group will include the matches in the result array
        // format: [text, token, text, token, text]
        const splitByToken = part.split(rule.regex);

        splitByToken.forEach((text, i) => {
          if (!text) return;

          // Odd indices are the captured tokens (matches)
          if (i % 2 === 1) {
            newParts.push(<span key={`${rule.type}-${partIndex}-${i}`} className={rule.className}>{text}</span>);
          } else {
            newParts.push(text);
          }
        });

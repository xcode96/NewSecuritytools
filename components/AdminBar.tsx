...
import { PlusIcon, XMarkIcon, PencilIcon, TrashIcon, UploadIcon, DownloadIcon, SettingsIcon, GitHubIcon, CodeIcon } from './IconComponents';
import type { Tool, ToolFormData, GitHubSettings, CategoryInfo } from '../types';
import GitHubSettingsModal from './GitHubSettingsModal';
import { publishToGitHub, fetchFromGitHub } from '../services/githubService';
...
const AdminBar: React.FC<AdminBarProps> = ({
  ...
  // Initialize tool category with first category if available
  useEffect(() => {
    if (categories.length > 0 && !toolCategory) {
      setToolCategory(categories[0].name);
    }
  }, [categories]);
  ...
  }, [editingTool, isAdding, categories]);
  ...
  const handleDownloadSource = () => {
    try {
      ...
      alert("An error occurred while generating the source file.");
    }
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-12">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 text-sm font-bold px-3 py-2 rounded-lg transition-colors bg-slate-800 text-white hover:bg-slate-700 dark:bg-indigo-500 dark:text-white dark:hover:bg-indigo-400"
              title="Download Source (constants.ts)"
            >

...
const BooksModal: React.FC<BooksModalProps> = ({ onClose, isOpen, isAdmin = false }) => {
    ...
    const stats = useMemo(() => {
        return {
            ...
            enterprise: books.filter(b => b.category === 'Enterprise Platforms').length,
            // Parse "300 pages" -> 300
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            let result;
            if (editForm.id) {
                // updateLink still returns boolean for now, need to be careful or update it too if we want consistecy.
                // For now, let's assume updateBook is legacy boolean or we check if we updated it?
                // Wait, I only updated addBook. updateBook is still boolean.
                // I should probably update updateBook too for consistency, but for this bug 'adding' is the issue.
                // Let's handle the mixed types.
                const success = await updateBook(editForm.id, payload);
                result = { success };
            } else {
                result = await addBook(payload);
            }
            ...
            if (result.success) {
                ...
                alert(`Failed to save book: ${result.error?.message || result.error?.details || JSON.stringify(result.error) || 'Unknown error'}`);
            }
        } catch (error) {
                    ...
                    <div className="flex flex-col xl:flex-row gap-6 items-start xl:items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            <div className="flex items-center gap-2 px-4 py-2 bg-white/50 dark:bg-white/5 rounded-full border border-slate-200 dark:border-white/10 shadow-sm backdrop-blur-sm">
                                ...
                                <span className="font-bold text-slate-700 dark:text-slate-200 text-sm">{stats.enterprise} Enterprise</span>
                            </div>
                            <div className="flex items-center gap-2 px-4 py-2 bg-white/50 dark:bg-white/5 rounded-full border border-slate-200 dark:border-white/10 shadow-sm backdrop-blur-sm">
                        ...
                        <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
                            ...
                            <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
                                {(['All', 'Blue Team', 'Red Team', 'Enterprise Platforms', 'General'] as const).map((cat) => (
                                    <button
                                        key={cat}
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${book.category === 'Red Team' ? 'bg-red-500/10 text-red-600 dark:text-red-400' :
                                            book.category === 'Blue Team' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400' :
                                                book.category === 'Enterprise Platforms' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400' :
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select value={editForm.category} onChange={e => setEditForm({ ...editForm, category: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 border-none focus:ring-2 focus:ring-amber-500">
                                            {['Blue Team', 'Red Team', 'Enterprise Platforms', 'General'].map(c => <option key={c} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    <div>
